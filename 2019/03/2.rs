    ...
    for x in (f.0).0..=(f.0).1 {
        for y in (f.1).0..=(f.1).1 {
            ...
            if space[i] == 'X' {
                ...
                println!("{}, {}, {}, {}, {:?}, {}", x, y, xx, yy, step, sum);
                min_steps = std::cmp::min(min_steps, sum);
            }
        }
    }
    min_steps
...
mod tests {
    ...
    #[test]
    fn test() {
        assert_eq!(solve(&vec!["R8,U5,L5,D3".to_string(), "U7,R6,D4,L4".to_string()]), 30);
        assert_eq!(solve(&vec!["R75,D30,R83,U83,L12,D49,R71,U7,L72".to_string(), "U62,R66,U55,R34,D71,R55,D58,R83".to_string()]), 610);
        assert_eq!(solve(&vec!["R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51".to_string(), "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7".to_string()]), 410);
    }
}
