...
struct Packet {
    version: u8,
    id: u8,
    data: (u32, u32), // (offs, len)
    value: Option<i64>,
    children: Vec<Box<Packet>>,
}

fn parse_packet(bytes: &Vec<u8>, bp: u32, c: usize) -> Packet {
    // println!("{:indent$}bp: {}", "", bp, indent=c*2);
    let version = bytes.get_u8(0, bp, 3).unwrap();
    let id = bytes.get_u8(0, bp + 3, 3).unwrap();
    // println!("{:indent$}parse: {}, {}, {}", "", bp, version, id, indent=c*2);
    match id {
        4u8 => {
            let mut pos = bp + 6;
            let mut num: i64 = 0;
            ...
            let plen = pos - (bp + 6);
            //println!("{:indent$}literal: {}, {}, {}, {}", "", num, bp + 6, pos, plen, indent=c*2);
            Packet {
                version,
                id,
                data: (bp + 6, plen),
        ...
        _ => {
            ...
            //println!("{:indent$}lt: {}", "", length_type, indent=c*2);
            match length_type {
                0 => {
                    ...
                    let mut packets = vec![];
                    while bits > 0 {
                        //println!("{:indent$}parse child.. bits left: {}", "", bits, indent=c*2);
                        let p = parse_packet(bytes, pos, c + 1);
                        pos += 6 + p.data.1;
                        bits -= 6 + p.data.1;
                        packets.push(Box::new(p));
			// Smallest packet is 11 bits
			if pos as usize + 11 >= bytes.len() * 8 {
			    //println!("{:indent$}no more data!", "", indent=c*2);
			    break;
			}
                    }
                    let plen = pos - (bp + 6);
                    //println!("{:indent$}children: {}, plen: {}", "", packets.len(), plen, indent=c*2);
                    Packet {
                        version,
                        id,
                        data: (bp + 6, plen),
                ...
                _ => {
                    ...
                    //println!("{:indent$}num: {}", "", num, indent=c*2);
                    let mut packets = vec![];
                    let mut pos = bp + 18;
                    for _i in 0..num {
                        //println!("{:indent$}parse child {}..", "", i, indent=c*2);
                        let p = parse_packet(bytes, pos, c + 1);
                        pos += 6 + p.data.1;
                        //println!("{:indent$}pos: {}", "", pos, indent=c*2);
                        packets.push(Box::new(p));
                    }
                    let plen = pos - (bp + 6);
                    //println!("{:indent$}children: {}", "", packets.len(), indent=c*2);
                    Packet {
                        version,
                        id,
                        data: (bp + 6, plen),
...
fn calculate(packet: &Packet) -> i64 {
	...
	0 => {
	    // sum
	    print!("sum(");
	    let v = packet.children.iter().map(|c| {
		let c = calculate(c);
		print!(",");
		c
	    }).sum();
	...
	4 => {
	    // literal
	    print!("{}", packet.value.unwrap() as i64);
	    packet.value.unwrap() as i64
	}
	5 => {
	    // gt
	    let left = calculate(&packet.children[0]);
	    print!(" > ");
	    let right = calculate(&packet.children[1]);
...
fn part1(bytes: &Vec<u8>) -> Answer {
    let packet = parse_packet(bytes, 0, 0);
    sum_version(&packet)
}
...
mod tests {
    ...
    fn test_parse_packet_1() {
        ...
        let p = parse_packet(&data, 0, 0);
        assert_eq!(p.id, 4u8);
        assert_eq!(p.version, 6u8);
        assert_eq!(p.data, (6, 15));
	...
	assert_eq!((*p.children[0]).value, Some(10));
	assert_eq!((*p.children[1]).value, Some(20));
        assert_eq!(p.data, (6, 43));
    }
	...
	assert_eq!((*p.children[0]).value, Some(1));
	assert_eq!((*p.children[1]).value, Some(2));
	assert_eq!((*p.children[2]).value, Some(3));
        assert_eq!(p.data, (6, 45));
    }
    ...
    #[test]
    fn test_part1_4() {
        assert_eq!(part1(&parse(&vec!["A0016C880162017C3686B18A3D4780".into()])), 31);
    }
